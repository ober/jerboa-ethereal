...
(def (dissect-xmcp buffer)
  ...
  (try
    (let* (
           ...
           (attr-service-version (unwrap (read-u32be buffer 4)))
           (attr-service-data (unwrap (slice buffer 4 1)))
           (attr-subscription-id (unwrap (read-u32be buffer 4)))
           (attr-domain (unwrap (read-u32be buffer 4)))
           (attr-value (unwrap (slice buffer 4 1)))
           (attr-length (unwrap (read-u16be buffer 6)))
