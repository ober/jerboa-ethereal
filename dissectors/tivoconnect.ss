...
;; jerboa-ethereal/dissectors/tivoconnect.ss
;; Auto-generated from wireshark/epan/dissectors/packet-tivoconnect.c
  ...
  (try
    (let* (
      ...
      (ok (list
        ...
        (cons 'machine (list (cons 'raw machine) (cons 'formatted (utf8->string machine))))
        (cons 'identity (list (cons 'raw identity) (cons 'formatted (utf8->string identity))))
        (cons 'services (list (cons 'raw services) (cons 'formatted (utf8->string services))))
        (cons 'version (list (cons 'raw version) (cons 'formatted (utf8->string version))))
        (cons 'flavor (list (cons 'raw flavor) (cons 'formatted (utf8->string flavor))))
        )))

    (catch (e)
      (err (str "TIVOCONNECT parse error: " e)))))

;; dissect-tivoconnect: parse TIVOCONNECT from bytevector
