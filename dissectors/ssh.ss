...
(def (dissect-ssh buffer)
  ...
  (try
    (let* (
           ...
           (hostkey-type (unwrap (slice buffer 21 1)))
           (hostkey-data (unwrap (slice buffer 21 1)))
           (hostsig-type (unwrap (slice buffer 29 1)))
           (hostsig-data-length (unwrap (read-u32be buffer 29)))
           (hostsig-data (unwrap (slice buffer 33 1)))
           (padding-length (unwrap (read-u8 buffer 37)))
           (payload (unwrap (slice buffer 39 1)))
           (padding-string (unwrap (slice buffer 39 1)))
           (dh-gex-min (unwrap (read-u32be buffer 45)))
           (dh-gex-nbits (unwrap (read-u32be buffer 49)))
           (dh-gex-max (unwrap (read-u32be buffer 53)))
           (hybrid-blob-client-len (unwrap (read-u32be buffer 59)))
           (hybrid-blob-server-len (unwrap (read-u32be buffer 63)))
           (packet-length (unwrap (read-u32be buffer 67)))
           (packet-length-encrypted (unwrap (slice buffer 67 4)))
           (encrypted-packet (unwrap (slice buffer 67 1)))
           (protocol (unwrap (slice buffer 67 1)))
           (cookie (unwrap (slice buffer 67 16)))
           (first-kex-packet-follows (unwrap (read-u8 buffer 83)))
           (kex-reserved (unwrap (slice buffer 84 4)))
           (kex-hassh-algo (unwrap (slice buffer 88 1)))
           (kex-hassh (unwrap (slice buffer 88 1)))
           (kex-hasshserver-algo (unwrap (slice buffer 88 1)))
           (kex-hasshserver (unwrap (slice buffer 88 1)))
