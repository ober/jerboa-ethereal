...
;; jerboa-ethereal/dissectors/pw-satop.ss
;; Auto-generated from wireshark/epan/dissectors/packet-pw_satop.c
;; RFC 4553
...
;; ── Dissector ──────────────────────────────────────────────────────
(def (dissect-pw-satop buffer)
  "SAToP"
  (try
    (let* (
           (l (unwrap (read-u32be buffer 0)))
           (hf-payload (unwrap (slice buffer 0 1)))
      ...
      (ok (list
        ...
        (cons 'rsv (list (cons 'raw rsv) (cons 'formatted (number->string rsv))))
        (cons 'bits03 (list (cons 'raw bits03) (cons 'formatted (number->string bits03))))
        (cons 'len (list (cons 'raw len) (cons 'formatted (number->string len))))
        (cons 'seq (list (cons 'raw seq) (cons 'formatted (number->string seq))))
        )))

    (catch (e)
      (err (str "PW-SATOP parse error: " e)))))

;; dissect-pw-satop: parse PW-SATOP from bytevector
