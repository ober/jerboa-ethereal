...
;; jerboa-ethereal/dissectors/opensafety.ss
;; Auto-generated from wireshark/epan/dissectors/packet-opensafety.c
...
;; ── Dissector ──────────────────────────────────────────────────────
(def (dissect-opensafety buffer)
  "openSAFETY"
  (try
    (let* (
           (snmt-udid (unwrap (slice buffer 0 6)))
           )

      (ok (list
        (cons 'snmt-udid (list (cons 'raw snmt-udid) (cons 'formatted (fmt-mac snmt-udid))))
        )))

    (catch (e)
      (err (str "OPENSAFETY parse error: " e)))))

;; dissect-opensafety: parse OPENSAFETY from bytevector
