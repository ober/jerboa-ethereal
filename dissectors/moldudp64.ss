...
;; jerboa-ethereal/dissectors/moldudp64.ss
;; Auto-generated from wireshark/epan/dissectors/packet-moldudp64.c
...
;; ── Dissector ──────────────────────────────────────────────────────
(def (dissect-moldudp64 buffer)
  "MoldUDP64"
  (try
    (let* (
           (sequence (unwrap (read-u64be buffer 0)))
           (count (unwrap (read-u16be buffer 0)))
           (session (unwrap (slice buffer 8 1)))
           )
    ...
    (catch (e)
      (err (str "MOLDUDP64 parse error: " e)))))

;; dissect-moldudp64: parse MOLDUDP64 from bytevector
