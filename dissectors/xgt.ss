...
(def (dissect-xgt buffer)
  ...
  (try
    (let* (
           ...
           (data-value-uint64 (unwrap (read-u64be buffer 24)))
           (data-value-uint8 (unwrap (read-u8 buffer 24)))
           (data-value-uint16 (unwrap (read-u16be buffer 24)))
           (data-value-uint32 (unwrap (read-u32be buffer 24)))
           (data (unwrap (slice buffer 24 1)))
           (reserved-area (unwrap (read-u16be buffer 28)))
           (data-length (unwrap (read-u16be buffer 34)))
           (status-data (unwrap (slice buffer 36 24)))
           (slot-info (unwrap (read-u32be buffer 36)))
           (cpu-type (unwrap (read-u16be buffer 40)))
           (ver-num (unwrap (read-u16be buffer 42)))
           (sys-state (unwrap (read-u32be buffer 44)))
           (padt-cnf (unwrap (read-u16be buffer 48)))
           (cnf-er (unwrap (read-u32be buffer 50)))
           (cnf-war (unwrap (read-u32be buffer 54)))
           (variable-count (unwrap (read-u16be buffer 60)))
           (block-count (unwrap (read-u16be buffer 60)))
           (company-id (unwrap (slice buffer 62 8)))
           )
