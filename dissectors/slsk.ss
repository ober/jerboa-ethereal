...
(def (dissect-slsk buffer)
  ...
  (try
    (let* (
           ...
           (total-uploads (unwrap (read-u32be buffer 183)))
           (queued-uploads (unwrap (read-u32be buffer 187)))
           (timestamp (unwrap (read-u32be buffer 240)))
           (chat-message-id (unwrap (read-u32be buffer 248)))
           (folder-count (unwrap (read-u32be buffer 284)))
           (file-count (unwrap (read-u32be buffer 288)))
           (compressed-packet-length (unwrap (read-u32be buffer 332)))
           (uncompressed-packet-length (unwrap (read-u32be buffer 332)))
           (size (unwrap (read-u32be buffer 369)))
           (num-strings (unwrap (read-u32be buffer 473)))
           (place-in-queue (unwrap (read-u32be buffer 485)))
           (number-of-rooms (unwrap (read-u32be buffer 505)))
           (bytes (unwrap (slice buffer 545 13)))
           (users-in-room (unwrap (read-u32be buffer 582)))
           (average-speed (unwrap (read-u32be buffer 586)))
           (download-number (unwrap (read-u32be buffer 590)))
           (files (unwrap (read-u32be buffer 598)))
           (directories (unwrap (read-u32be buffer 602)))
           (num-slotsfull-records (unwrap (read-u32be buffer 606)))
           (slotsfull (unwrap (read-u32be buffer 610)))
           (code (unwrap (read-u32be buffer 618)))
           (number-of-priv-users (unwrap (read-u32be buffer 642)))
           (parent-min-speed (unwrap (read-u32be buffer 663)))
           (parent-speed-connection-ratio (unwrap (read-u32be buffer 671)))
           (seconds-parent-inactivity-before-disconnect (unwrap (read-u32be buffer 679)))
           (seconds-server-inactivity-before-disconnect (unwrap (read-u32be buffer 687)))
           (nodes-in-cache-before-disconnect (unwrap (read-u32be buffer 695)))
           (seconds-before-ping-children (unwrap (read-u32be buffer 703)))
           (number-of-days (unwrap (read-u32be buffer 719)))
           (embedded-message-type (unwrap (read-u32be buffer 727)))
           (byte (unwrap (read-u8 buffer 740)))
           (num-parent-address (unwrap (read-u32be buffer 745)))
           (ip (unwrap (read-u32be buffer 749)))
           (port (unwrap (read-u32be buffer 753)))
           (number-of-users (unwrap (read-u32be buffer 781)))
           (same-recommendation (unwrap (read-u32be buffer 785)))
           (ranking (unwrap (read-u32be buffer 801)))
           (num-recommendations (unwrap (read-u32be buffer 813)))
           (integer (unwrap (read-u32be buffer 834)))
           (connection-type (unwrap (slice buffer 843 4)))
           (token (unwrap (read-u32be buffer 852)))
           (message-code (unwrap (read-u32be buffer 856)))
           )
