...
(def (dissect-wsp buffer)
  ...
  (try
    (let* (
           ...
           (content-md5 (unwrap (slice buffer 0 1)))
           (warning-agent (unwrap (slice buffer 0 1)))
           (warning-text (unwrap (slice buffer 0 1)))
           (content-range-first-byte-pos (unwrap (read-u32be buffer 0)))
           (content-range-entity-length (unwrap (read-u32be buffer 0)))
           (range-first-byte-pos (unwrap (read-u32be buffer 0)))
           (range-last-byte-pos (unwrap (read-u32be buffer 0)))
           (range-suffix-length (unwrap (read-u32be buffer 0)))
      ...
      (ok (list
        ...
        (cons 'content-md5 (list (cons 'raw content-md5) (cons 'formatted (fmt-bytes content-md5))))
        (cons 'warning-agent (list (cons 'raw warning-agent) (cons 'formatted (utf8->string warning-agent))))
        (cons 'warning-text (list (cons 'raw warning-text) (cons 'formatted (utf8->string warning-text))))
        (cons 'content-range-first-byte-pos (list (cons 'raw content-range-first-byte-pos) (cons 'formatted (number->string content-range-first-byte-pos))))
        (cons 'content-range-entity-length (list (cons 'raw content-range-entity-length) (cons 'formatted (number->string content-range-entity-length))))
        (cons 'range-first-byte-pos (list (cons 'raw range-first-byte-pos) (cons 'formatted (number->string range-first-byte-pos))))
        (cons 'range-last-byte-pos (list (cons 'raw range-last-byte-pos) (cons 'formatted (number->string range-last-byte-pos))))
        (cons 'range-suffix-length (list (cons 'raw range-suffix-length) (cons 'formatted (number->string range-suffix-length))))
