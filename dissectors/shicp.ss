...
(def (dissect-shicp buffer)
  ...
  (try
    (let* (
           ...
           (supported-msg (unwrap (slice buffer 5 1)))
           (hn-max-len (unwrap (read-u8 buffer 5)))
           (pswd-max-len (unwrap (read-u8 buffer 5)))
           (challenge (unwrap (read-u32be buffer 5)))
           (validity-period (unwrap (read-u8 buffer 5)))
           (error (unwrap (read-u8 buffer 5)))
           (ip (unwrap (read-u32be buffer 7)))
           (sn (unwrap (read-u32be buffer 7)))
           (gw (unwrap (read-u32be buffer 7)))
           (dns1 (unwrap (read-u32be buffer 7)))
           (dns2 (unwrap (read-u32be buffer 7)))
           (dhcp (unwrap (read-u8 buffer 7)))
           (hn (unwrap (slice buffer 7 1)))
           (pswd (unwrap (slice buffer 7 1)))
           (token (unwrap (slice buffer 7 1)))
           (wink-type (unwrap (read-u8 buffer 7)))
           )
