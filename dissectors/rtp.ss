...
(def (dissect-rtp buffer)
  ...
  (try
    (let* (
           ...
           (padding-data (unwrap (slice buffer 37 1)))
           (padding-count (unwrap (read-u8 buffer 37)))
           (marker (unwrap (read-u8 buffer 40)))
           (payload-type (unwrap (read-u8 buffer 40)))
           (seq-nr (unwrap (read-u16be buffer 40)))
           (timestamp (unwrap (read-u32be buffer 42)))
           (ssrc (unwrap (read-u32be buffer 46)))
           (csrc-item (unwrap (read-u32be buffer 62)))
           (length (unwrap (read-u16be buffer 68)))
           (hdr-ext (unwrap (read-u32be buffer 70)))
           )
