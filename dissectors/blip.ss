...
;; jerboa-ethereal/dissectors/blip.ss
;; Auto-generated from wireshark/epan/dissectors/packet-blip.c
...
;; ── Dissector ──────────────────────────────────────────────────────
(def (dissect-blip buffer)
  "BLIP Couchbase Mobile"
  (try
    (let* (
           (message-body (unwrap (slice buffer 0 1)))
           (message-number (unwrap (read-u64be buffer 1)))
           )

      (ok (list
        (cons 'message-body (list (cons 'raw message-body) (cons 'formatted (utf8->string message-body))))
        (cons 'message-number (list (cons 'raw message-number) (cons 'formatted (number->string message-number))))
        )))

    (catch (e)
      (err (str "BLIP parse error: " e)))))

;; dissect-blip: parse BLIP from bytevector
