...
;; jerboa-ethereal/dissectors/rsh.ss
;; Auto-generated from wireshark/epan/dissectors/packet-rsh.c
...
(def (dissect-rsh buffer)
  ...
  (try
    (let* (
           ...
           (client-username (unwrap (slice buffer 0 1)))
           (server-username (unwrap (slice buffer 0 1)))
      ...
      (ok (list
        ...
        (cons 'client-username (list (cons 'raw client-username) (cons 'formatted (utf8->string client-username))))
        (cons 'server-username (list (cons 'raw server-username) (cons 'formatted (utf8->string server-username))))
        (cons 'command (list (cons 'raw command) (cons 'formatted (utf8->string command))))
        )))

    (catch (e)
      (err (str "RSH parse error: " e)))))

;; dissect-rsh: parse RSH from bytevector
