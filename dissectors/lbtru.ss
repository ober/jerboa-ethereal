...
;; jerboa-ethereal/dissectors/lbtru.ss
;; Auto-generated from wireshark/epan/dissectors/packet-lbtru.c
...
;; ── Dissector ──────────────────────────────────────────────────────
(def (dissect-lbtru buffer)
  "LBT Reliable Unicast Protocol"
  (try
    (let* (
           (analysis-next-rst-frame (unwrap (read-u32be buffer 0)))
           (analysis-prev-rst-frame (unwrap (read-u32be buffer 0)))
           (analysis-next-creq-frame (unwrap (read-u32be buffer 0)))
           (analysis-prev-creq-frame (unwrap (read-u32be buffer 0)))
           (analysis-next-ack-frame (unwrap (read-u32be buffer 0)))
           (analysis-prev-ack-frame (unwrap (read-u32be buffer 0)))
           ...
           (channel-client (unwrap (read-u32be buffer 0)))
           (channel-id (unwrap (read-u64be buffer 0)))
           (tag (unwrap (slice buffer 0 1)))
           )

      (ok (list
        (cons 'analysis-next-rst-frame (list (cons 'raw analysis-next-rst-frame) (cons 'formatted (number->string analysis-next-rst-frame))))
        (cons 'analysis-prev-rst-frame (list (cons 'raw analysis-prev-rst-frame) (cons 'formatted (number->string analysis-prev-rst-frame))))
        (cons 'analysis-next-creq-frame (list (cons 'raw analysis-next-creq-frame) (cons 'formatted (number->string analysis-next-creq-frame))))
        (cons 'analysis-prev-creq-frame (list (cons 'raw analysis-prev-creq-frame) (cons 'formatted (number->string analysis-prev-creq-frame))))
        (cons 'analysis-next-ack-frame (list (cons 'raw analysis-next-ack-frame) (cons 'formatted (number->string analysis-next-ack-frame))))
        (cons 'analysis-prev-ack-frame (list (cons 'raw analysis-prev-ack-frame) (cons 'formatted (number->string analysis-prev-ack-frame))))
        ...
        (cons 'channel-client (list (cons 'raw channel-client) (cons 'formatted (number->string channel-client))))
        (cons 'channel-id (list (cons 'raw channel-id) (cons 'formatted (fmt-hex channel-id))))
        (cons 'tag (list (cons 'raw tag) (cons 'formatted (utf8->string tag))))
        )))

    (catch (e)
      (err (str "LBTRU parse error: " e)))))

;; dissect-lbtru: parse LBTRU from bytevector
