...
;; jerboa-ethereal/dissectors/oicq.ss
;; Auto-generated from wireshark/epan/dissectors/packet-oicq.c
...
;; ── Dissector ──────────────────────────────────────────────────────
(def (dissect-oicq buffer)
  "OICQ - IM software, popular in China"
  (try
    (let* (
           (version (unwrap (read-u16be buffer 1)))
           (seq (unwrap (read-u16be buffer 5)))
           (qqid (unwrap (read-u32be buffer 7)))
           (data (unwrap (slice buffer 11 1)))
           )
      ...
      (ok (list
        ...
        (cons 'qqid (list (cons 'raw qqid) (cons 'formatted (number->string qqid))))
        (cons 'data (list (cons 'raw data) (cons 'formatted (utf8->string data))))
        )))

    (catch (e)
      (err (str "OICQ parse error: " e)))))

;; dissect-oicq: parse OICQ from bytevector
