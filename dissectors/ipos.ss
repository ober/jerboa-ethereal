...
;; jerboa-ethereal/dissectors/ipos.ss
;; Auto-generated from wireshark/epan/dissectors/packet-ipos.c
...
;; ── Dissector ──────────────────────────────────────────────────────
(def (dissect-ipos buffer)
  "IPOS Kernel Packet Protocol"
  (try
    (let* (
           (priority (unwrap (read-u8 buffer 0)))
           (slot (unwrap (read-u16be buffer 2)))
           )
    ...
    (catch (e)
      (err (str "IPOS parse error: " e)))))

;; dissect-ipos: parse IPOS from bytevector
