...
;; jerboa-ethereal/dissectors/pw-fr.ss
;; Auto-generated from wireshark/epan/dissectors/packet-pw_fr.c
;; RFC 4619
...
;; ── Dissector ──────────────────────────────────────────────────────
(def (dissect-pw-fr buffer)
  "PW Frame Relay DLCI Control Word"
  (try
    (let* (
           (cr (unwrap (read-u8 buffer 0)))
           (de (unwrap (read-u8 buffer 0)))
           (becn (unwrap (read-u8 buffer 0)))
           (fecn (unwrap (read-u8 buffer 0)))
      ...
      (ok (list
        ...
        (cons 'de (list (cons 'raw de) (cons 'formatted (number->string de))))
        (cons 'becn (list (cons 'raw becn) (cons 'formatted (number->string becn))))
        (cons 'fecn (list (cons 'raw fecn) (cons 'formatted (number->string fecn))))
    ...
    (catch (e)
      (err (str "PW-FR parse error: " e)))))

;; dissect-pw-fr: parse PW-FR from bytevector
