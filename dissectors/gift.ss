...
;; jerboa-ethereal/dissectors/gift.ss
;; Auto-generated from wireshark/epan/dissectors/packet-gift.c
...
;; ── Dissector ──────────────────────────────────────────────────────
(def (dissect-gift buffer)
  "giFT Internet File Transfer"
  (try
    (let* (
           (response (unwrap (read-u8 buffer 0)))
           (request (unwrap (read-u8 buffer 0)))
           (request-cmd (unwrap (slice buffer 0 1)))
           (response-cmd (unwrap (slice buffer 0 1)))
      ...
      (ok (list
        ...
        (cons 'request-cmd (list (cons 'raw request-cmd) (cons 'formatted (utf8->string request-cmd))))
        (cons 'response-cmd (list (cons 'raw response-cmd) (cons 'formatted (utf8->string response-cmd))))
    ...
    (catch (e)
      (err (str "GIFT parse error: " e)))))

;; dissect-gift: parse GIFT from bytevector
