...
;; jerboa-ethereal/dissectors/mrp-msrp.ss
;; Auto-generated from wireshark/epan/dissectors/packet-mrp_msrp.c
...
;; ── Dissector ──────────────────────────────────────────────────────
(def (dissect-mrp-msrp buffer)
  "Multiple Stream Reservation Protocol"
  (try
    (let* (
           (end-mark (unwrap (read-u16be buffer 0)))
           )

      (ok (list
        (cons 'end-mark (list (cons 'raw end-mark) (cons 'formatted (fmt-hex end-mark))))
        )))

    (catch (e)
      (err (str "MRP-MSRP parse error: " e)))))

;; dissect-mrp-msrp: parse MRP-MSRP from bytevector
