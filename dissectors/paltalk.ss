...
;; jerboa-ethereal/dissectors/paltalk.ss
;; Auto-generated from wireshark/epan/dissectors/packet-paltalk.c
...
;; ── Dissector ──────────────────────────────────────────────────────
(def (dissect-paltalk buffer)
  "Paltalk Messenger Protocol"
  (try
    (let* (
           (pdu-type (unwrap (read-u16be buffer 0)))
           (version (unwrap (read-u16be buffer 2)))
           (length (unwrap (read-u16be buffer 4)))
           (content (unwrap (slice buffer 6 1)))
           )
    ...
    (catch (e)
      (err (str "PALTALK parse error: " e)))))

;; dissect-paltalk: parse PALTALK from bytevector
