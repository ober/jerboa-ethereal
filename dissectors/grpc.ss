...
;; jerboa-ethereal/dissectors/grpc.ss
;; Auto-generated from wireshark/epan/dissectors/packet-grpc.c
...
;; ── Dissector ──────────────────────────────────────────────────────
(def (dissect-grpc buffer)
  "GRPC Message"
  (try
    (let* (
           (message-data (unwrap (slice buffer 0 1)))
           (message-length (unwrap (read-u32be buffer 1)))
           )
    ...
    (catch (e)
      (err (str "GRPC parse error: " e)))))

;; dissect-grpc: parse GRPC from bytevector
