...
;; jerboa-ethereal/dissectors/netgear-ensemble.ss
;; Auto-generated from wireshark/epan/dissectors/packet-netgear_ensemble.c
...
;; ── Dissector ──────────────────────────────────────────────────────
(def (dissect-netgear-ensemble buffer)
  "Netgear Ensemble Protocol"
  (try
    (let* (
           (tlv-length (unwrap (read-u32be buffer 0)))
           (version (unwrap (read-u8 buffer 0)))
           (unknown (unwrap (slice buffer 1 3)))
           (sequence (unwrap (read-u32be buffer 4)))
           (uuid (unwrap (slice buffer 25 16)))
           (ip (unwrap (read-u32be buffer 41)))
           (mac (unwrap (slice buffer 65 6)))
           (unknown-int32 (unwrap (read-u32be buffer 75)))
           (uptime (unwrap (read-u32be buffer 95)))
           )
      ...
      (ok (list
        ...
        (cons 'unknown-int32 (list (cons 'raw unknown-int32) (cons 'formatted (number->string unknown-int32))))
        (cons 'uptime (list (cons 'raw uptime) (cons 'formatted (number->string uptime))))
        )))

    (catch (e)
      (err (str "NETGEAR-ENSEMBLE parse error: " e)))))

;; dissect-netgear-ensemble: parse NETGEAR-ENSEMBLE from bytevector
