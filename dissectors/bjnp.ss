...
;; jerboa-ethereal/dissectors/bjnp.ss
;; Auto-generated from wireshark/epan/dissectors/packet-bjnp.c
...
;; ── Dissector ──────────────────────────────────────────────────────
(def (dissect-bjnp buffer)
  "Canon BJNP"
  (try
    (let* (
           (no (unwrap (read-u32be buffer 4)))
           (len (unwrap (read-u32be buffer 10)))
           (hf-payload (unwrap (slice buffer 14 1)))
           (id (unwrap (slice buffer 15 4)))
           )
    ...
    (catch (e)
      (err (str "BJNP parse error: " e)))))

;; dissect-bjnp: parse BJNP from bytevector
