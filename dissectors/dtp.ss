...
;; jerboa-ethereal/dissectors/dtp.ss
;; Auto-generated from wireshark/epan/dissectors/packet-dtp.c
...
;; ── Dissector ──────────────────────────────────────────────────────
(def (dissect-dtp buffer)
  "Dynamic Trunk Protocol"
  (try
    (let* (
           (version (unwrap (read-u8 buffer 0)))
           (tlvlength (unwrap (read-u16be buffer 3)))
           (domain (unwrap (slice buffer 5 1)))
           (senderid (unwrap (slice buffer 5 6)))
           (data (unwrap (slice buffer 5 6)))
           )
      ...
      (ok (list
        ...
        (cons 'senderid (list (cons 'raw senderid) (cons 'formatted (fmt-mac senderid))))
        (cons 'data (list (cons 'raw data) (cons 'formatted (fmt-mac data))))
        )))

    (catch (e)
      (err (str "DTP parse error: " e)))))

;; dissect-dtp: parse DTP from bytevector
