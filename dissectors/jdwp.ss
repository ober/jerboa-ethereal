...
;; jerboa-ethereal/dissectors/jdwp.ss
;; Auto-generated from wireshark/epan/dissectors/packet-jdwp.c
...
;; ── Dissector ──────────────────────────────────────────────────────
(def (dissect-jdwp buffer)
  "Java Debug Wire Protocol"
  (try
    (let* (
           (length (unwrap (read-u32be buffer 0)))
           (id (unwrap (read-u32be buffer 4)))
           (flags (unwrap (read-u8 buffer 8)))
           (data (unwrap (slice buffer 31 1)))
           (type (unwrap (slice buffer 32 14)))
           )
    ...
    (catch (e)
      (err (str "JDWP parse error: " e)))))

;; dissect-jdwp: parse JDWP from bytevector
