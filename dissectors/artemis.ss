...
;; jerboa-ethereal/dissectors/artemis.ss
;; Auto-generated from wireshark/epan/dissectors/packet-artemis.c
...
;; ── Dissector ──────────────────────────────────────────────────────
(def (dissect-artemis buffer)
  "Artemis Core Protocol"
  (try
    (let* (
           (len (unwrap (read-u32be buffer 0)))
           (type (unwrap (read-u8 buffer 4)))
           (channel (unwrap (read-u64be buffer 5)))
           (buffer (unwrap (slice buffer 13 1)))
           )
      ...
      (ok (list
        ...
        (cons 'buffer (list (cons 'raw buffer) (cons 'formatted (fmt-bytes buffer))))
        )))

    (catch (e)
      (err (str "ARTEMIS parse error: " e)))))

;; dissect-artemis: parse ARTEMIS from bytevector
