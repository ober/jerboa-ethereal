...
;; jerboa-ethereal/dissectors/trel.ss
;; Auto-generated from wireshark/epan/dissectors/packet-trel.c
...
;; ── Dissector ──────────────────────────────────────────────────────
(def (dissect-trel buffer)
  "TREL Protocol"
  (try
    (let* (
           (rsv (unwrap (read-u8 buffer 0)))
           (ack (unwrap (read-u8 buffer 0)))
           (type (unwrap (read-u8 buffer 0)))
           (channel (unwrap (read-u8 buffer 0)))
           (dest-panid (unwrap (read-u16be buffer 0)))
           (packetno (unwrap (read-u32be buffer 2)))
      ...
      (ok (list
        ...
        (cons 'dest-panid (list (cons 'raw dest-panid) (cons 'formatted (number->string dest-panid))))
        (cons 'packetno (list (cons 'raw packetno) (cons 'formatted (number->string packetno))))
        (cons 'version (list (cons 'raw version) (cons 'formatted (number->string version))))
        )))

    (catch (e)
      (err (str "TREL parse error: " e)))))

;; dissect-trel: parse TREL from bytevector
